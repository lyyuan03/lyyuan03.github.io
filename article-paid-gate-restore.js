...
function installStyles() {
  ...
  style.textContent = `
    ...
    [data-paid-gate-restored] .paid-lock-card{position:relative;z-index:2;width:min(540px,92%);margin:0 auto;padding:26px 24px 22px;text-align:center;background:rgba(248,244,236,.98);border:1px solid rgba(165,130,84,.36);box-shadow:0 16px 38px rgba(42,34,26,.12);color:#3F3024}
    [data-paid-gate-restored] .member-lock-icon{margin-bottom:7px;color:#8B683F;font-size:18px;line-height:1}
    [data-paid-gate-restored] h3{margin:0 0 10px;color:#493724;font-family:'Noto Serif TC',serif;font-size:24px;font-weight:700;letter-spacing:.05em}
    [data-paid-gate-restored] .paid-lock-card>p{margin:0 auto 15px;max-width:455px;color:#665747;font-size:13px;line-height:1.9}
    [data-paid-gate-restored] .paid-value-note{margin:11px auto 0;max-width:455px;color:#765C40;font-size:11px;line-height:1.75}
    ...
    @media(max-width:560px){[data-paid-gate-restored] .paid-lock-card{width:94%;padding:22px 16px}[data-paid-gate-restored] h3{font-size:22px}}
  `;
  ...
  return `
    <section class="member-lock-zone paid-lock-zone" data-paid-gate-restored aria-label="解鎖贊助專屬全文">
      ...
      <div class="member-lock-card paid-lock-card sponsor-join-card">
        ...
        <h3>解鎖全文</h3>
        <p>本篇前段開放試閱。選擇 1 個月或 3 個月閱讀方案後，在有效期間內可閱讀所有贊助專屬文章。</p>
        <div data-sponsor-smart-slot data-sponsor-context="article">
          <div class="sponsor-offer-loading">閱讀方案載入中，請稍候。</div>
        </div>
        <div class="paid-value-note">付款完成後系統會自動開通閱讀資格，並帶你回到這篇文章繼續閱讀。</div>
        ...
        <small>請使用付款時登入的 Gmail 閱讀全文。</small>
      </div>
    </section>`;
}

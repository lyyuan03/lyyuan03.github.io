...
function errorMessage(error) {
  ...
  if (code.includes("not-found")) return "找不到這篇文章的後台紀錄，請先確認文章已儲存。";
...
function updateButtonState() {
  ...
  [previewButton, notifyButton].forEach((button) => {
    ...
    button.title = enabled ? "" : "文章發布後才能使用通知功能";
  });
  if (!enabled) setNotificationStatus("文章發布後，即可預覽並寄送上架通知。", "");
...
function openPreviewWindow() {
  ...
  if (previewWindow) {
    previewWindow.document.write("<!doctype html><html lang='zh-Hant'><meta charset='utf-8'><title>文章通知信預覽</title><body style='font-family:sans-serif;padding:24px'>正在產生通知信預覽…</body></html>");
...
async function refreshNotificationDeliveryStatus() {
  ...
  try {
    ...
    if (data.status === "sent") {
      ...
      setNotificationStatus(`已寄送上架通知｜共 ${count} 位收件者`, "success");
      ...
      setNotificationStatus(detail ? `寄送失敗｜${detail}` : "寄送失敗，請檢查通知服務狀態。", "error");
    } else {
      setNotificationStatus("尚未寄送上架通知。", "");
    }
  } catch (error) {
...
async function handlePreview() {
  ...
  setNotificationStatus("正在產生通知信預覽…", "saving");
  try {
...
async function handleNotify() {
  ...
  notifyButton.disabled = true;
  previewButton.disabled = true;
  setNotificationStatus("正在確認目前有閱讀資格的會員…", "saving");
  try {
    const preview = await getPreview(articleId);
    const recipientCount = Number(preview.recipientCount || 0);
    if (recipientCount < 1) {
      setNotificationStatus("目前沒有符合這篇文章閱讀資格的收件者。", "error");
      showToast("目前沒有符合閱讀資格的會員，因此未寄送通知。", "error");
      return;
    }
    const confirmed = window.confirm(
      `確定要寄送這篇文章的上架通知嗎？\n\n文章：${preview.title || form?.elements?.title?.value || "目前文章"}\n收件對象：${preview.recipientScope || "依文章閱讀權限判定"}\n預計收件人數：${recipientCount.toLocaleString("zh-TW")} 位\n\n信件內會提供「閱讀完整文章」按鈕。每篇文章成功寄送後只能通知一次。`
    );
    if (!confirmed) {
    ...
    setNotificationStatus("上架通知寄送中…", "saving");
...
function installNotificationControls() {
  ...
  notifyButton.textContent = "寄送上架通知";

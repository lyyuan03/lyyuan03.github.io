(() => {
  ...
  const setProductCopy = (key, badge, description, cta) => {
    const card = document.querySelector(`[data-product="${key}"]`);
    if (!card) return;
    const badgeElement = card.querySelector('.badge');
    const descriptionElement = card.querySelector('.product-desc');
    const ctaElement = card.querySelector('.product-cta span');
    if (badgeElement) badgeElement.textContent = badge;
    if (descriptionElement) descriptionElement.textContent = description;
    if (ctaElement) ctaElement.textContent = cta;
  };

  function applyGalleryCopy() {
    const meta = document.querySelector('meta[name="description"]');
    if (meta) meta.content = '靈元院靈性美學館，收錄祈願御守與鎮煞護安香粉，讓虔敬、守護與日常修持安住於生活之中。';

    const heroLead = document.querySelector('.hero-copy .hero-lead');
    if (heroLead) heroLead.innerHTML = '讓祈願有所依止，<br>讓守護安住於日常。';
    ...
    const introCopy = document.querySelector('.intro .section-head p:not(.section-en)');
    if (introCopy) introCopy.textContent = '靈性美學館所選之物，並非單純裝飾，而是承接祈願、提醒修持的日常助緣。願每一次看見、佩戴與使用，都能讓人收攝心念，記得自己所發的願，也記得以正念行於人間。';
    ...
    promises.slice(0, 3).forEach((item, index) => {
      const title = item.querySelector('b');
      const copy = item.querySelector('span');
      if (title) title.textContent = promiseCopy[index][0];
      if (copy) copy.textContent = promiseCopy[index][1];
    });

    setText('.products .section-head h2', '靈元院祈願選物');
    const productsCopy = document.querySelector('.products .section-head p:not(.section-en)');
    if (productsCopy) productsCopy.textContent = '收錄四款祈願御守與鎮煞護安香粉。請先閱讀各品項的祈願方向，再依此刻所願與實際需要選擇；點選商品即可前往綠界選購。';
    const incenseFilter = document.querySelector('.filter-btn[data-filter="incense"]');
    if (incenseFilter) incenseFilter.textContent = '護安香品';
    ...
    if (note) note.textContent = '御守與香品皆為祈願與日常修持之助緣，並非取代個人的判斷、行動與責任。商品價格、會員方案、庫存、使用方式與出貨規範，請以綠界表單當下內容為準。';
    ...
    steps.slice(0, 3).forEach((step, index) => {
      ...
      if (title) title.textContent = stepCopy[index][0];
      if (copy) copy.textContent = stepCopy[index][1];
    });
  ...
  function patchQuizGeneratedText() {
    const elements = document.querySelectorAll('#quizQuestion,.quiz-option,#resultEnergy,#resultLead,#resultCopy,.axis-pill,.radar-label');
    elements.forEach(element => {
      ...
      if (value.includes('護身結界')) value = value.replace('護身結界', '護身守正');
      if (element.textContent.trim() !== value) element.textContent = value;
    });
  }

  applyGalleryCopy();
  patchQuizGeneratedText();

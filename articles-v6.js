...
function applyDragonChantOverrides() {
  ...
  if (cover && cover.getAttribute("src") !== DRAGON_CHANT_PRIMARY_IMAGE) cover.setAttribute("src", DRAGON_CHANT_PRIMARY_IMAGE);
  const body = detail?.querySelector(".article-body");
  if (!body) return;
  const walker = document.createTreeWalker(body, NodeFilter.SHOW_TEXT);
  const nodes = [];
  while (walker.nextNode()) nodes.push(walker.currentNode);
  nodes.forEach((node) => { if (node.nodeValue?.includes("*")) node.nodeValue = node.nodeValue.replace(/\*/g, ""); });
}
...
async function loadArticleCore() {
  ...
  for (const [index, moduleUrl] of coreModuleUrls.entries()) {
    try {
      ...
      lastError = error;
      if (index === 0) console.warn("文選核心首次載入失敗，正在重試。", error);
    }
  }
...
async function loadArticleAddons() {
  const addons = [
    ...
    ["非會員贊助方案", "./article-paid-gate-restore.js?v=20260923-ghosts-supernatural-1"],
    ["付費正文安全載入", "./paid-article-secure-loader.js?v=20260923-ghosts-supernatural-1"]
  ];
  ...
  results.forEach((result, index) => {
    if (result.status === "rejected") console.warn(`${addons[index][0]}附加模組載入失敗，文章本體維持正常顯示。`, result.reason);
  });
}
...
if (!document.getElementById(articleVisualFixStyleId)) {
  ...
  style.textContent = `
    ...
    .article-card-content .article-engagement b { color: #3F3024 !important; text-shadow: none !important; }
    .article-card-content .article-list-title, .article-card-content h2 { font-weight: 700 !important; }
    .article-card-content .article-hook { color: #493F36 !important; }
    .article-card-content .article-meta { color: #725532 !important; }
    .article-card-content .article-access-badge.is-free { color: #4F5228 !important; background: rgba(96,99,48,.13) !important; border-color: rgba(96,99,48,.32) !important; }
    .article-card-content .article-access-badge.is-paid { color: #6A4D2E !important; background: rgba(165,130,84,.14) !important; border-color: rgba(139,104,63,.34) !important; }
    .article-card-content .article-access-badge.is-event { color: #594F47 !important; background: rgba(89,79,71,.10) !important; border-color: rgba(89,79,71,.30) !important; }
    .footer-brand-mark { display: block; width: 128px; max-width: 34vw; height: auto; opacity: .64; filter: saturate(.72) brightness(.78); }
  `;

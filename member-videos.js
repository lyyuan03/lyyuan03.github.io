...
function hasLingjiAccess() {
  if (isAdminEmail(currentUser?.email)) return true;
  const expiry = dateValue(currentAccess?.expiresAt);
  return currentAccess?.status === "active" && currentAccess?.level === "lingji" && expiry && expiry > new Date();
}
...
async function loadVideos() {
  ...
  try {
    ...
    console.warn("靈極影片暫時無法載入。", error);
  }
  render(videos);
}

function render(videos) {
  const allowed = hasLingjiAccess();
  if (!videos.length) {
    root.innerHTML = '<div class="video-empty">目前尚未發布靈極會員影片。</div>';
    return;
  }
  root.innerHTML = `<div class="video-grid">${videos.map((video) => {
    const id = youtubeId(video.youtubeUrl || video.youtubeId || "");
    const title = escapeHtml(video.title || "靈極會員專屬影片");
    ...
    if (allowed && id) {
      return `<article class="video-card is-open"><div class="video-frame"><iframe src="https://www.youtube-nocookie.com/embed/${encodeURIComponent(id)}?rel=0&modestbranding=1" title="${title}" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe></div><div class="video-copy"><span>${escapeHtml(dateLabel)}</span><h2>${title}</h2>${description ? `<p>${description}</p>` : ""}</div></article>`;
    }
    return `<article class="video-card is-locked"><div class="video-lock"><span>靈極會員專屬</span><strong>${title}</strong><p>${currentUser ? "目前帳號沒有有效的靈極會員資格，或資格已到期。" : "請先使用登記的 Gmail 登入；有效的靈極會員即可觀看全部歷年影片。"}</p><button type="button" data-member-login>${currentUser ? "重新確認會員資格" : "會員登入"}</button></div></article>`;
  }).join("")}</div>`;

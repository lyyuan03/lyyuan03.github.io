(() => {
  ...
  const classifyLink = (anchor) => {
    ...
    const context =
      normalizeText(anchor.closest("article, section, .card, .book-rec")?.querySelector("h1,h2,h3,h4,h5")?.textContent) ||
      text;
    ...
    if (host === "books.com.tw" || host.endsWith(".books.com.tw")) {
      return {
        event: "click_books_com_tw",
        parameters: {
          book_name: context,
          link_text: text,
          link_url: url.href,
          destination_platform: "books.com.tw",
    ...
    if (
      location.pathname.endsWith("/books.html") &&
      (/購書|購買|簽名書/.test(text) || isExternal)
    ) {
      return {
        event: "click_books_buy",
        parameters: {
          book_name: context,
          link_text: text,
          link_url: url.href,
          destination_platform: host,
          button_location: location.pathname
        }
      };
    }
    ...
    if (/課程/.test(text)) {
      return { event: "click_course", parameters: { link_text: text, link_url: url.href } };
    }

    if (/會員|登入/.test(text)) {
      return { event: "click_membership", parameters: { link_text: text, link_url: url.href } };
    }

    if (/報名|法會|活動/.test(text) || host === "beclass.com") {
      return {
        event: "click_event_registration",
        parameters: { link_text: text, link_url: url.href, destination_platform: host }
      };
    }
  ...
  const trackPageType = () => {
    ...
    if (path.endsWith("/books.html")) {
      push("view_books_page", { page_category: "books" });
    }
    if (path.endsWith("/articles.html") && new URLSearchParams(location.search).has("id")) {
      push("view_article_detail", {
        article_id: new URLSearchParams(location.search).get("id")
      });
    }
  };
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      if (loginButton) {
        push("member_login", {
          button_text: normalizeText(loginButton.textContent),
          button_location: location.pathname
        });
      }
